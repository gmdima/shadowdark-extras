 ...
 */
export function generateSpellConfig(MODULE_ID, flags, effectsListHtml, effectsArray, effectsApplyToTarget, summonsList, summonProfilesArray) {
	...
	return damageConfig + summoningConfig;
}

/**
 * Generate Potion damage config (applies to drinker, uses caster variables)
 */
export function generatePotionConfig(MODULE_ID, flags, effectsListHtml, effectsArray, effectsApplyToTarget) {
	return generateSpellDamageConfigHTML(MODULE_ID, flags, effectsListHtml, effectsArray, effectsApplyToTarget, {
		targetLabel: 'Drinker',
 ...
 */
export function generateScrollConfig(MODULE_ID, flags, effectsListHtml, effectsArray, effectsApplyToTarget) {
	return generateSpellDamageConfigHTML(MODULE_ID, flags, effectsListHtml, effectsArray, effectsApplyToTarget, {
		targetLabel: 'TARGET',
		showTargetOption: true,
		requirementExamples: '@target.level < 3, @target.hp > 10, @level >= 5',
 ...
 */
export function generateWandConfig(MODULE_ID, flags, effectsListHtml, effectsArray, effectsApplyToTarget) {
	return generateSpellDamageConfigHTML(MODULE_ID, flags, effectsListHtml, effectsArray, effectsApplyToTarget, {
		targetLabel: 'TARGET',
		showTargetOption: true,
		requirementExamples: '@target.level < 3, @target.hp > 10, @level >= 5',
